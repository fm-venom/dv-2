...
export const Builds: React.FC<Props> = ({ currentUser, onNotification }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const buildsData = await storage.getBuilds();
        const usersData = await storage.getUsers();
        const userLikesData = await storage.getUserLikes(currentUser.id);
        
        setBuilds(buildsData);
        setUsers(usersData);
        setUserLikes({ [currentUser.id]: userLikesData });
      } catch (error) {
        console.error('Error loading data:', error);
        onNotification('Ошибка загрузки данных', 'error');
      }
    };
  ...
  const handleLike = async (buildId: string) => {
    ...
    try {
      if (hasLiked) {
        await storage.unlikeBuild(currentUser.id, buildId);
        setUserLikes({
          ...userLikes,
          [currentUser.id]: currentLikes.filter(id => id !== buildId)
        });
        setBuilds(builds.map(build => 
          build.id === buildId ? { ...build, likes: build.likes - 1 } : build
        ));
      } else {
        await storage.likeBuild(currentUser.id, buildId);
        setUserLikes({
          ...userLikes,
          [currentUser.id]: [...currentLikes, buildId]
        });
        setBuilds(builds.map(build => 
          build.id === buildId ? { ...build, likes: build.likes + 1 } : build
        ));
      ...
      onNotification('Ошибка при обработке лайка', 'error');
    }
  };

  const handleView = async (buildId: string) => {
    try {
      await storage.incrementBuildViews(buildId);
      setBuilds(builds.map(build => 
        build.id === buildId ? { ...build, views: build.views + 1 } : build
      ));
    } catch (error) {
  ...
  const handleSubmitBuild = async () => {
    ...
    try {
      await storage.createPendingBuild({
      ...
      onNotification('Ошибка при отправке билда', 'error');
    }
  };
